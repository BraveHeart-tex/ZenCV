...
// Sections that live in the sidebar (left dark column)
...
export const TokyoTemplate = ({
  ...
  const renderSidebarSection = (section: (typeof sections)[number]) => {
    if (section.type === INTERNAL_SECTION_TYPES.SKILLS) {
      return <TokyoSkillsSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.LANGUAGES) {
      return <TokyoLanguagesSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.WEBSITES_SOCIAL_LINKS) {
      return <TokyoLinksSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.HOBBIES) {
      return <TokyoHobbiesSection section={section} key={section.id} />;
    }
    return null;
  };
  ...
  const renderMainSection = (section: (typeof sections)[number]) => {
    if (section.type === INTERNAL_SECTION_TYPES.WORK_EXPERIENCE) {
      return <TokyoWorkExperienceSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.EDUCATION) {
      return <TokyoEducationSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.INTERNSHIPS) {
      return <TokyoInternshipsSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.COURSES) {
      return <TokyoCoursesSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.CUSTOM) {
      return <TokyoCustomSection section={section} key={section.id} />;
    }
    if (section.type === INTERNAL_SECTION_TYPES.REFERENCES) {
      return <TokyoReferencesSection section={section} key={section.id} />;
    }
    return null;
  };
  ...
  return (
    <Document>
      <Page size='A4' style={tokyoTemplateStyles.page}>
        {/* Sidebar */}
        <View style={tokyoTemplateStyles.sidebar}>
          <TokyoPersonalDetailsSection personalDetails={personalDetails} />
          {sidebarSections.map(renderSidebarSection)}
        </View>

        {/* Main content */}
        <View style={tokyoTemplateStyles.main}>
          <TokyoSummarySection summarySection={summarySection} />
