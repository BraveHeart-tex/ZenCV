...
import {
  ...
  getFieldsWithItemIds,
} from '@/lib/client-db/fieldService';
import {
  bulkAddItems,
  bulkDeleteItems,
...
export async function createDocument(
  ...
  return clientDb.transaction(
    ...
    async () => {
      ...
      const sectionInsertIds = await bulkAddSections(
      ...
      const itemInsertIds = await bulkAddItems(itemInsertDtos);
      ...
      await bulkAddFields(resolvedFieldDtos);
...
export async function getFullDocumentStructure(
...
export async function deleteDocument(documentId: DEX_Document['id']) {
  return clientDb.transaction(
    ...
    async () => {
      ...
      const sectionIds = await getSectionIdsByDocumentId(documentId);
      await bulkDeleteSections(sectionIds);

      const itemIds = await getItemIdsBySectionIds(sectionIds);
...
export async function changeDocumentTemplateType(
  documentId: DEX_Document['id'],
...
export async function copyDocument(documentId: DEX_Document['id']) {
  const sourceDocumentStructure = await getFullDocumentStructure(documentId);
  ...
  return clientDb.transaction(
    ...
    async () => {
      ...
      const itemInsertIds = await bulkAddItems(
      ...
      await bulkAddFields(flatFields);
