...
- **Child ticket**: a GitHub sub-issue linked to the map. Where sub-issues are unavailable, use a map task list and add `Part of #<map>` to the child.
- **Blocking**: GitHub native issue dependencies. Where unavailable, use a `Blocked by: #<n>` line.
